...
const $ = dom.$;
const increaseHoverVerbosityIcon = registerIcon('hover-increase-verbosity', Codicon.add, nls.localize(1141, 'Icon for increaseing hover verbosity.'));
const decreaseHoverVerbosityIcon = registerIcon('hover-decrease-verbosity', Codicon.remove, nls.localize(1142, 'Icon for decreasing hover verbosity.'));
...
let MarkdownHoverParticipant = class MarkdownHoverParticipant {
    ...
    createLoadingMessage(anchor) {
        return new MarkdownHover(this, anchor.range, [new MarkdownString().appendText(nls.localize(1143, "Loading..."))], false, 2000);
    }
    computeSync(anchor, lineDecorations) {
        ...
        if (stopRenderingLineAfter >= 0 && lineLength > stopRenderingLineAfter && anchor.range.startColumn >= stopRenderingLineAfter) {
            ...
            result.push(new MarkdownHover(this, anchor.range, [{
                    value: nls.localize(1144, "Rendering paused for long line for performance reasons. This can be configured via `editor.stopRenderingLineAfter`.")
                }], false, index++));
        }
        if (!stopRenderingMessage && typeof maxTokenizationLineLength === 'number' && lineLength >= maxTokenizationLineLength) {
            result.push(new MarkdownHover(this, anchor.range, [{
                    value: nls.localize(1145, "Tokenization is skipped for long lines for performance reasons. This can be configured via `editor.maxTokenizationLineLength`.")
                }], false, index++));
        }
...
export function labelForHoverVerbosityAction(keybindingService, action) {
    switch (action) {
        case HoverVerbosityAction.Increase:
            return keybindingService.appendKeybinding(nls.localize(1146, "Increase Hover Verbosity"), INCREASE_HOVER_VERBOSITY_ACTION_ID);
        case HoverVerbosityAction.Decrease:
            return keybindingService.appendKeybinding(nls.localize(1147, "Decrease Hover Verbosity"), DECREASE_HOVER_VERBOSITY_ACTION_ID);
    }
}
