...
export const warningIcon = registerIcon('extensions-warning-message', Codicon.warning, nls.localize(1553, 'Icon shown with a warning message in the extensions editor.'));
        ...
        this._updateState = (state) => {
            if (state && state.hasMore) {
                ...
                if (state.nonBasicAsciiCharacterCount >= max) {
                    data = {
                        message: nls.localize(1554, 'This document contains many non-basic ASCII unicode characters'),
                        command: new DisableHighlightingOfNonBasicAsciiCharactersAction(),
                    };
                }
                else if (state.ambiguousCharacterCount >= max) {
                    data = {
                        message: nls.localize(1555, 'This document contains many ambiguous unicode characters'),
                        command: new DisableHighlightingOfAmbiguousCharactersAction(),
                    };
                }
                else if (state.invisibleCharacterCount >= max) {
                    data = {
                        message: nls.localize(1556, 'This document contains many invisible unicode characters'),
...
const configureUnicodeHighlightOptionsStr = nls.localize(1557, 'Configure Unicode Highlight Options');
let UnicodeHighlighterHoverParticipant = class UnicodeHighlighterHoverParticipant {
    ...
    computeSync(anchor, lineDecorations) {
        ...
        for (const d of lineDecorations) {
            ...
            switch (highlightInfo.reason.kind) {
                case 0 /* UnicodeHighlighterReasonKind.Ambiguous */: {
                    if (isBasicASCII(highlightInfo.reason.confusableWith)) {
                        reason = nls.localize(1558, 'The character {0} could be confused with the ASCII character {1}, which is more common in source code.', codePointStr, formatCodePointMarkdown(highlightInfo.reason.confusableWith.codePointAt(0)));
                    }
                    else {
                        reason = nls.localize(1559, 'The character {0} could be confused with the character {1}, which is more common in source code.', codePointStr, formatCodePointMarkdown(highlightInfo.reason.confusableWith.codePointAt(0)));
                    }
                    break;
                }
                case 1 /* UnicodeHighlighterReasonKind.Invisible */:
                    reason = nls.localize(1560, 'The character {0} is invisible.', codePointStr);
                    break;
                case 2 /* UnicodeHighlighterReasonKind.NonBasicAscii */:
                    reason = nls.localize(1561, 'The character {0} is not a basic ASCII character.', codePointStr);
                    break;
            }
            ...
            const adjustSettings = nls.localize(1562, 'Adjust settings');
...
export class DisableHighlightingInCommentsAction extends EditorAction {
    constructor() {
        super({
            ...
            label: nls.localize2(1571, "Disable highlighting of characters in comments"),
            precondition: undefined
        });
        this.shortLabel = nls.localize(1563, 'Disable Highlight In Comments');
    }
...
export class DisableHighlightingInStringsAction extends EditorAction {
    constructor() {
        super({
            id: DisableHighlightingOfAmbiguousCharactersAction.ID,
            label: nls.localize2(1572, "Disable highlighting of characters in strings"),
            precondition: undefined
        });
        this.shortLabel = nls.localize(1564, 'Disable Highlight In Strings');
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
export class DisableHighlightingOfAmbiguousCharactersAction extends Action2 {
    ...
    constructor() {
        super({
            id: DisableHighlightingOfAmbiguousCharactersAction.ID,
            title: nls.localize2(1573, "Disable highlighting of ambiguous characters"),
            precondition: undefined,
            f1: false,
        });
        this.shortLabel = nls.localize(1565, 'Disable Ambiguous Highlight');
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
export class DisableHighlightingOfInvisibleCharactersAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: nls.localize2(1574, "Disable highlighting of invisible characters"),
            precondition: undefined,
            f1: false,
        });
        this.shortLabel = nls.localize(1566, 'Disable Invisible Highlight');
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
export class DisableHighlightingOfNonBasicAsciiCharactersAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: nls.localize2(1575, "Disable highlighting of non basic ASCII characters"),
            precondition: undefined,
            f1: false,
        });
        this.shortLabel = nls.localize(1567, 'Disable Non ASCII Highlight');
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
export class ShowExcludeOptions extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: nls.localize2(1576, "Show Exclude Options"),
            precondition: undefined,
            f1: false,
        });
    }
    async run(accessor, args) {
        ...
        function getExcludeCharFromBeingHighlightedLabel(codePoint) {
            if (InvisibleCharacters.isInvisibleCharacter(codePoint)) {
                return nls.localize(1568, 'Exclude {0} (invisible character) from being highlighted', codePointToHex(codePoint));
            }
            return nls.localize(1569, 'Exclude {0} from being highlighted', `${codePointToHex(codePoint)} "${char}"`);
        }
        const options = [];
        if (reason.kind === 0 /* UnicodeHighlighterReasonKind.Ambiguous */) {
            for (const locale of reason.notAmbiguousInLocales) {
                options.push({
                    label: nls.localize(1570, "Allow unicode characters that are more common in the language \"{0}\".", locale),
                    run: async () => {
